$(function(){
    ...
    /*
    body.on('click', '#selectPostType li a', function(){
        $('#selectPostType').find('li').each(function(){
            $(this).removeClass('active');
        });
        $(this).parent().addClass('active');
        $('#item-type').val($(this).data('type'));
        if (($(this).data('type') == '2') || ($(this).data('type') == '3')) {
            $('.field-item-text').hide();
            if ($(this).data('type') == '3') {
                $('.field-item-video_url').show();
            } else {
                $('.field-item-video_url').hide();
            }
        } else {
            $('.field-item-video_url').hide();
            $('.field-item-text').show();
        }
